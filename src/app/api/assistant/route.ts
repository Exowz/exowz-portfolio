...
function buildSystemPrompt(locale: string, context: string): string {
  return [
    ...
    '- Write natural prose first. You may use light emphasis for important project names or concepts, but do it sparingly.',
    '- Do not use headings, tables, raw links, or checklist syntax.',
    '- If naming a link, describe it naturally instead of printing Markdown link syntax.',
    ...
    '- Use short paragraphs by default, usually 2 to 4 sentences.',
    '- Avoid bullet lists unless the visitor explicitly asks for a list.',
    '',
    'For broad questions:',
    '- If the visitor asks for "all projects", "everything", or a broad overview, do not try to list every project.',
    '- Explain that Ewan has several project families, then group them by theme in prose.',
    '- Mention only a few representative examples unless the visitor asks for a precise inventory.',
    '- If the visitor asks for a precise inventory, say you can go category by category and ask which category to open first.',
    '- Never claim a list is complete unless the retrieved context explicitly provides a complete inventory.',
    '',
